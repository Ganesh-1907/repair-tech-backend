...
const CONTRACT_BUCKET = 'cmcContracts';
const CONTRACT_PREFIX = 'CMC';
...
export const listCmcContracts = async ({ search, status, fromDate, toDate, page, limit }) => {
...
export const getCmcContractById = async (id) => getRecord(CONTRACT_BUCKET, id);

export const createCmcContract = async (payload) => {
  const nextPayload = {
    ...payload,
    contractType: 'CMC',
    cmcDetails: payload.cmcDetails || payload,
    coverageType: payload.coverageType || payload.cmcDetails?.coverageType || 'Comprehensive',
    partsIncluded: payload.partsIncluded ?? payload.cmcDetails?.partsIncluded ?? true,
  };
  return saveRecord(CONTRACT_BUCKET, nextPayload, CONTRACT_PREFIX);
};

export const updateCmcContract = async (id, payload) => {
  const nextPayload = {
    ...payload,
    id,
    contractType: 'CMC',
...
export const deleteCmcContract = async (id) => removeRecord(CONTRACT_BUCKET, id);

export const updateCmcContractStatus = async (id, status) => {
...
export const exportCmcContractsCsv = async (query) => {
  const { data } = await listCmcContracts({ ...query, page: 1, limit: Number.MAX_SAFE_INTEGER });
